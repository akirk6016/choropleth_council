...
## Filtering out Oahu
intersected_oahu <- intersected_polygons %>% 
  filter(mokupuni == "O'ahu")
## Summing landuse coverage per ahupua'a
oahu_landuse_sum <- intersected_oahu %>% 
  group_by(moku, resample) %>% 
  summarize(landuse_area = sum(st_areasha)) %>% 
  ungroup()

ggplot() + 
  geom_sf(data = oahu_landuse_sum, aes(fill = resample, color = moku)) + 
