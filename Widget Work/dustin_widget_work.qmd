...
landuse_sf <- st_as_sf(landuse) %>% 
  ...
  mutate(resample = case_when(
    ...
    landcover == "51" ~ "Waterway",
    landcover == "52" ~ "Waterway",
    landcover == "53" ~ "Waterway",
    landcover == "54" ~ "Waterway",
...
ggplot() + 
  geom_sf(data = landuse_sf, aes(fill = resample)) + 
  theme(legend.position = "none")

view(unique(landuse_sf$landcover))

# Areas that are zero are urban areas 
#   places that are zero for food and carbon are urban areas 
#   
...
### Testing kiauea vs kilauea 
data_vec_test <- data_sf_clean %>% 
  filter(ahupuaa == "Kilauea" | ahupuaa == "Kiauea")
data_vec_test1 <- data_sf_clean %>% 
  filter(ahupuaa == "Kilauea")
data_vec_test2 <- data_sf_clean %>% 
  filter(ahupuaa == "Kiauea")
vect(data_vec_test)
vect(data_vec_test1)
vect(data_vec_test2)
ggplot() +
  geom_sf(data = data_vec_test1, fill = "blue")
ggplot() +
  geom_sf(data = data_vec_test2, fill = "red")
ggplot() +
  geom_sf(data = data_vec_test, aes(fill = ahupuaa))
```



```{r}
# Cropping my multi-layer spatraster to the extent of my filte#882222 vector 
...
ggplot() +
  ...
  geom_tile(data = multi_layer_maui_df, inherit.aes = FALSE, aes(x = x, y = y), fill = "#882222")
