...
# Filtering data to just hawaii 
data_vec2 <- data_sf_clean %>% filter( mokupuni == "Hawai'i") 
# Vectorizing it 
vect(data_vec2)
...
# values <- extract(multi_layer_rs, data_vect, fun = max, na.rm = TRUE)
...
r2 <- crop(multi_layer_rs, extent(data_vec2))
# Converting my cropped r2 raster to a dataframe 
r2_df <- as.data.frame(r2, xy = TRUE)

# PLotting it over the filtered vector 
ggplot() +
  geom_sf(data = data_vec2) +
  geom_tile(data = r2_df, inherit.aes = FALSE, aes(x = x, y = y), fill = "red")

# Plotting my different rasters that are selected for food = 1 and carbon = 1 over the islands data 
ggplot() +
  geom_sf(data = data_vect) +
  geom_tile(data = multi_layer_df_food, inherit.aes = FALSE, aes(x = x, y = y), fill = "red") + 
